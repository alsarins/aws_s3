...
func (h *ProxyHandler) PreRequestEncryptionHook(r *http.Request, innerRequest *http.Request, info *BucketInfo) (*CountingHash, error) {
	// если это не PUT запрос или не задан info.Config.EncryptionKey то возвращаем nil и выходим из функции
	if info == nil || info.Config == nil || info.Config.EncryptionKey == "" || r.Method != "PUT" {
		return nil, nil
	}

	// когда мы здесь, значит задано шифрование PUT запроса
...
func (h *ProxyHandler) SignRequest(r *http.Request, info *BucketInfo) error {
	...
	if dateStr == "" && r.Header.Get("x-amz-date") == "" {
		...
		// dateStr = time.Now().UTC().Format(time.RFC7231)
...
func (h *ProxyHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// semamaev: копируем http request для рассчета оригинального md5 для PUT запроса
	originalBodyData, _ := ioutil.ReadAll(r.Body)
	r.Body = ioutil.NopCloser(bytes.NewBuffer(originalBodyData)) // Восстанавливаем r.Body
	...
	if dataCheckNeeded {
		...
		_, _ = io.Copy(originalBodyHashStatic, reqCopy.Body)
	}
	...
	// TODO в случае без шифрования у нас баг - паника. Разобраться!
	if dataCheckNeeded {
		...
		innerEtag := fmt.Sprintf("\"%x\"", bodyHash.Sum(nil))
		...
		originalEtag := fmt.Sprintf("\"%x\"", originalBodyHashStatic.Sum(nil))

		// if the Etags don't match, we can leave whatever value there
		// здесь мы проверяем что получили от сервера тот же md5 который и посылали (на шифрованный или нешифрованный Body)
		if innerEtag == awsEtag {
			innerResponse.Header["Etag"] = []string{originalEtag}
		}

		// ^^^ так мы добились того, что когда команда PUT отрабатывает, то на клиента возвращается оригинальный MD5 в случае с шифрованием
...
func main() {
	...
	if *debugMode {
		...
		InfoLogger.Print("Enabling debug messages. Version 1_94")
	}
